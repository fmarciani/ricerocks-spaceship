...
class Sprite:
    ...
    def draw(self, canvas):
        ...
        if self.animated == True:
            canvas.draw_image(self.image, [self.image_center[0] + self.age * 
...
def rock_spawner():
    ...
    rock_vel = [random.random() * (score / 50) * .6 - .3, random.random() * (score / 50) * .6 - .3]
    rock_avel = random.random() * .2 - .1
    a_rock = Sprite(rock_pos, rock_vel, 0, rock_avel, asteroid_image, asteroid_info)
    rock_group.add(a_rock)
    
    # make sure there are only 12 rocks at a time on the canvas
    # make sure rocks don't spawn on top of ship
